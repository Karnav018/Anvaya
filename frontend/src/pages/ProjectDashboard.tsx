        ...
        <div className="space-y-4">
          {loading ? (
             ...
             <div className="text-center py-10 border border-dashed border-[#1c2028] rounded-xl bg-[#10131a]">
               ...
               <p className="text-[12px] text-[#73757d] mb-4">
                 {canEdit ? 'Create your first project to orchestrate a backend API.' : 'No projects yet in this workspace.'}
               </p>
               {canEdit && (
                 <button
                    onClick={() => setIsModalOpen(true)}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {projects.map(p => (
                 <WorkflowCard
                   ...
                   status="live"
                   onClick={() => navigate(`/o/${orgSlug}/editor/${p.id}`)}
                 />
               ))}
             </div>
          )}
